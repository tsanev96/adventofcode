...
function breakTxtInput(args: string[]): Boats {
  const times = args[0]
    .split(" ")
    .filter((n) => n !== "" && !Number.isNaN(Number(n)))
    .map((n) => Number(n));
  const recordDistances = args[1]
    .split(" ")
    .filter((n) => n !== "" && !Number.isNaN(Number(n)))
    .map((n) => Number(n));
...
function createBoats({ times, recordDistances }: Boats) {
  const boats: SingleBoat[] = [];
...
function getCountToBeatRecord(boats: SingleBoat[]) {
  const waysToBeatRecord: number[] = [];

  boats.forEach(({ time, recordDistance }) => {
...
function boatRaces(args: string[]) {
  const { times, recordDistances } = breakTxtInput(args);

  const boats = createBoats({ times, recordDistances });

  const arrWaysToBeatRecord = getCountToBeatRecord(boats);
